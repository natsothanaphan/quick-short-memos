...
app.post('/api/memos', async (req, res) => {
  ...
  if (!timestamp || !title || !content) return res.status(400).json({ error: 'Timestamp, title, content are required' });
  const memoTimestamp = new Date(timestamp);
  if (isNaN(memoTimestamp.getTime())) return res.status(400).json({ error: 'Invalid timestamp' });
  memoTimestamp.setSeconds(0, 0); // round to minute

  const memoData = {
    timestamp: memoTimestamp, title, content,
    createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp(),
  };
  try {
    const docRef = await db
      .collection('users').doc(req.uid)
      .collection('memos').add(memoData);
...
app.get('/api/memos', async (req, res) => {
  ...
  if (!dayStr) return res.status(400).json({ error: 'Day is required' });
  if (dayStr.length !== 10) return res.status(400).json({ error: 'Invalid day format. Expected format: YYYY-MM-DD' });
  const rawTargetDate = new Date(dayStr);
  if (isNaN(rawTargetDate.getTime())) return res.status(400).json({ error: 'Invalid day format. Expected format: YYYY-MM-DD' });
  const timezone = req.query.timezone;
  if (!timezone) return res.status(400).json({ error: 'Timezone is required' });
  const timezoneOffset = parseInt(timezone, 10);
  if (isNaN(timezoneOffset)) return res.status(400).json({ error: 'Invalid timezone parameter. Expected integer representing minutes' });
...
app.patch('/api/memos/:id', async (req, res) => {
  ...
  if (req.body.timestamp) {
    ...
    if (isNaN(updatedTimestamp.getTime())) return res.status(400).json({ error: 'Invalid timestamp' });
  ...
  try {
    ...
    if (!doc.exists) return res.status(404).json({ error: 'Memo not found' });
